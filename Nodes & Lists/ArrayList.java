...
public class ArrayList implements List {
    ...
    /*
    * removes all occurences of item in the array
    */
    public boolean removeAll(Object item) {
        int count = 0;

        boolean print = false;
        Object[] newArray = new Object[length]; //new object length (length)
        
        for (int i = 0; i < length; i++) {
            if (items[i].equals(item)) {  //for every object, if the object equals to the item
                print = true; //set print to true
            }
            else {
                newArray[count] = items[i]; //otherwise, new array takes the item i
                count++;
            }
        }
        items = newArray; //items points to the new array
