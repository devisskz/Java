...
public class LLList implements List {
    ...
    public boolean removeAll(Object item) {
        ...
        Node q = head;
        Node c = head;
        Node node = new Node(null,null); //new node points to null
        
        
        
        for (Node p = head; p !=null; p = p.next) { //loops through each object

            

            if (p.item == item) { // if current object is the item
                
                print = true; //set print to true

                q = q.next; // the q pointer is now on next item
                
            }
            Node newNode = new Node(q.item,null);
            
            //newNode = q; //if the item is not what we have to remove, then new node with this item
            c = newNode;
            node.next = c; //the next node is now the new item.
            
            
        }
        head = node; //head now points to node
            
            return print; //returns true or false
        }
