...
public class StringNode {
    ...
    public static StringNode copy(StringNode str) {
        StringNode point = null;                         // initialize pointers to null
        StringNode point2 = null;                        
        int count = 0;                                  // initialize count to 0
        if (str == null) {                              // base cose, if input is null
            return null;
        }


        for (StringNode p = str; p != null; p = p.next) {         // for loop going through each object
            StringNode copyFirst = new StringNode(p.ch, null);    // creates a new node every call
            if (count == 0) {                                     // when count is initially 0, 
                point = copyFirst;                                 // sets copy to the frist pointer of copyfrist
            }
            
            if (count != 0) {                                     // after count is not 0 (after first call)
                point2.next = copyFirst;                           // the next pointer in pointer 2 points to the adress of copyFirst
            }
            point2 = copyFirst;                                    // then current pointer 2 is set to current copyFirst
            count++;
            
        }
        return point;                                              //returns the untouched pointer that still points to the whole new StringNode
    }
    ...
    public static int indexOf(StringNode str, char ch) {
        int countIndex = 0;  //initialize index count to 0
        
        if (str == null) { //if str is null, return -1
            return -1;
        }
        while (str!=null) {              //while loop
            if (str.ch == ch) {          //if current str.ch == ch
                return countIndex;        //returns the current index
            }
            countIndex++;      // otherwise, increases index by 1
            str = str.next;  //str is now the next node
        } 
   ...
   // changed from iterative to recursive

    public static StringNode insertAfter(StringNode str, char newChar, char afterChar) {

        StringNode charNode = new StringNode(newChar, null);        // create new node of charNode

        if (str == null) {                                          // base case, if str is null
            str = charNode;                                         // set str to new char node
            return str;                                             
        }
        if (str.ch == afterChar) {                                  // if ch is the indicated one
            charNode.next = str.next;                               // set created ch node to the next str
            str.next = charNode;                                    // set the str to node
            return str;
        }
        
        str.next = insertAfter(str.next, newChar, afterChar);   // recursive call to str.next pointer
    ...
    public static boolean isPrefix(StringNode prefix, StringNode str) {
        if (str==null){
            return false;                           // base case, if input is null
        }
        
        while (str != null && prefix != null) {     // while str and prefix are not null
            if (prefix.ch != str.ch) {              // if current ch is not equal, false
                return false;
            }
            str = str.next;                         // otherwise, continues to next
            prefix = prefix.next;                   // ^ continues to next until prefix is null
        }

        return prefix == null;                      // if prefix reaches null, returns true, otherwise false
    }
    ...
    public static StringNode removeAllSpaces(StringNode str) {
        StringNode prev = str;        // initialize a node prev that is going to hold the pointer
                                      // to the previous object
        if (str == null) {            // base case, if str = null, returns null
            return null;
        }
        for (StringNode p = str; p != null; p = p.next ) { // loops through each object in list
            if (p.ch == ' ') {                             // if the value of the object is a space
                prev.next = p.next;                        // sets the pointer prev from previous
                                                           // object straight to the next object
            }
                                                           // if in this iteration p.ch is not a space
            prev = p;                                      // set pointer of previous object to current
            
        }
        return str;                                        // returns a pointer to str (from beginning of the list)
    }
    ...
    public static void toUpperCase(StringNode str) {
        ...
        if (str == null) {     // base case if null -> null
            return;             // does nothing
        }
        
        str.ch = Character.toUpperCase(str.ch);  // sets current ch in str to upper cse
        str = str.next;                          // sets pointer str to next object
        toUpperCase(str);                         // recursive call
