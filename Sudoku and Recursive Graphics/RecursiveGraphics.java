...
public class RecursiveGraphics extends Canvas  {
    ...
    private static final int windowSize = 900;     // height and width of window in pixels
   ...
   private double[] thetaDelta = {Math.PI/5, -Math.PI/5};   // add this delta to angle each time -- in radians
    ...
    private void drawFractalLine(double x1, double y1, double x5, double y5, int depth, Graphics g) {
	   ...
	   //       (x1,y1) --- (x2,y2)     (x4,y4) --  (x5,y5) 
           ...
           // Your code here
           for (int i = 0; i < 1; i++) {
            drawFractalLine(x1, y1, x2, y2, depth-1, g); //recursive call on point x1>x2 y1>y2
           }
            for (int i = 0; i < 1; i++) {
                drawFractalLine(x2, y2, x3, y3, depth-1, g); //recursive call on next point
        }
        for (int i = 0; i < 1; i++) {
            drawFractalLine(x3, y3, x4, y4, depth-1, g); //recursive call on next point
           }
           for (int i = 0; i < 1; i++) {
            drawFractalLine(x4, y4, x5, y5, depth-1, g); //recursive call on last point
           } //all lines connect and draw the designated shape
       }
   }
    ...
    // Your code here
    drawFractalLine(x, y, x+s, y, depth, g); //along the length of the x,y x+s, y+s etc
    drawFractalLine(x+s, y, x+s, y+s, depth, g);
    drawFractalLine(x+s, y+s, x, y+s,depth, g);
    drawFractalLine(x, y+s, x, y,depth, g);
    }   //4 calls from one point to other, all in order so that call of 2 will draw the wanted drawing
    ...
    // Draw a filled triangle using the points in X and Y
        ...
        drawLine(X[0], Y[0], X[2], Y[2], g); 
        drawLine(X[0], Y[0], X[1], Y[1], g);
        drawLine(X[2], Y[2], X[1], Y[1], g);
         
       
	// Now calculate the midpoints in each side, as described in the video
	
	double y_mid = (Y[0]+Y[2])/2; //pointon the y mid
	double x_left = (X[2]+X[0])/2; //point on x_left
    double x_right = (X[0]+X[1])/2; //point on the x_right

	
        g.setColor(Color.RED); //i couldnt figure out how to make the shape filled in, so for visual i made the lines red
	
        // Now recursively fill in middle sub-triangle in white
        for (int i = 0; i < 4; i++) {
            drawLine(x_left, y_mid, x_right, y_mid, g); //draws line recursively from point to point
            drawLine(x_left, y_mid, X[0], Y[1]+Y[0]/90, g);
            drawLine(x_right, y_mid, X[0], Y[0]/90+Y[1], g);
        }
